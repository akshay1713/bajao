...
class MusicLibrary():
    ...
    def add_to_library(self, directory, songs):
        ...
        with self.conn:
            ...
            for song in songs:
                ...
                song_db_details = self.get_song_db_details(
                    abs_song_path, directory)
                if song_db_details["exists"]:
                    print("The song " + os.path.basename(abs_song_path) +
                ...
                cursor.execute("INSERT INTO MusicFiles(file_name, directory_id, uniqueid) values(?, ?, ?)", (
                    song_name, song_db_details["directory_id"], unique_song_hash))
    ...
    def get_song_db_details(self, abs_song_path, directory):
        ...
        unique_song_hash = hashlib.md5(
            abs_song_path.encode('utf-8')).hexdigest()[0:15]
        exists_check_result = self.check_if_exists(
            unique_song_hash, abs_song_path)
        song_db_details.update(exists_check_result)
        return song_db_details

    def get_directory_id(self, directory):
        with self.conn:
            existing_dir = self.conn.execute(
                "SELECT id from MusicDirectories where directory = (?)", [directory]).fetchone()
            if existing_dir is None:
                cursor = self.conn.cursor()
                cursor.execute(
                    "INSERT INTO MusicDirectories(directory) values(?)", [directory])
                return cursor.lastrowid
            return existing_dir[0]

    def check_if_exists(self, uniqueid, abs_song_path):
        with self.conn:
            existing_uniqueid = self.conn.execute(
                "SELECT file_name from MusicFiles where uniqueid = (?)", [uniqueid]).fetchone()
            ...
            new_hash = hashlib.md5(
                abs_song_path.encode('utf-8')).hexdigest()[16:31]
    ...
    def create_playlist(self, playlist_name):
        with self.conn:
            playlist = self.conn.execute(
                "SELECT * from Playlists where playlist_name = (?)", [playlist_name]).fetchone()
            if playlist is not None:
                print("A playlist with the name ", playlist_name,
                      " already exists. Please use another name")
                return
            cursor = self.conn.cursor()
            cursor.execute(
                "INSERT INTO Playlists(playlist_name) values(?)", [playlist_name])
            if cursor.lastrowid:
    ...
    def add_to_playlist(self, song_name, playlist_name):
        playlist_entry_details = self.get_playlist_entries(
    ...
    def get_playlist_entries(self, song_name, playlist_name):
        ...
        with self.conn:
            playlist_details = self.conn.execute(
                "SELECT * from Playlists where playlist_name = (?)", [playlist_name]).fetchone()
            ...
            song_details = self.conn.execute(
                "SELECT rowid from MusicFiles where file_name = (?)", [base_song_name]).fetchone()
            if song_details is None:
                if not os.path.isfile(song_name):
                    print(
                        "song ", song_name, "not found in either the music library or the path specified.")
                    return
                print("song ", song_name,
                      "found, adding it to Music Library before adding it to playlist.")
                song_id_array = self.add_to_library(
                    [base_song_name], os.path.dirname(song_name))
